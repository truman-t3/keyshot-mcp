...
describe("output path safety", () => {
  ...
  it("places relative paths and legal subdirectories under the output directory", async () => {
    expect(await normalizeOutputPath(config(), "preview.png")).toBe(
      path.join(outputDir, "preview.png"),
    );
    expect(
      await normalizeOutputPath(config(), path.join("renders", "preview.png")),
    ).toBe(path.join(outputDir, "renders", "preview.png"));
  });
  ...
  it.each([
    "parent traversal",
    "adjacent same-prefix directory",
    "external absolute path",
  ])("rejects %s", async (label) => {
    const target =
    ...
    await expect(normalizeOutputPath(config(), target)).rejects.toThrow(
      "KEYSHOT_OUTPUT_DIR",
    );
  });
  ...
  it("allows external paths only with the compatibility switch", async () => {
    ...
    expect(
      await normalizeOutputPath(
        config({ keyshotAllowExternalOutputs: true }),
        target,
      ),
    ).toBe(target);
  });
  ...
  it("rejects a symlink or junction that escapes the output directory", async () => {
    ...
    await expect(
      normalizeOutputPath(config(), path.join("linked", "escape.png")),
    ).rejects.toThrow("KEYSHOT_OUTPUT_DIR");
  });

  it.skipIf(process.platform !== "win32")(
    "handles Windows drive-letter casing consistently",
    async () => {
      ...
      await expect(normalizeOutputPath(config(), swapped)).resolves.toBe(
        swapped,
      );
    },
  );
